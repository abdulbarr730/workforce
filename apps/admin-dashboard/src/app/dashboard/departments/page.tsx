    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{deptList.length} total departments</p>
        </div>
        <button
          onClick={() => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {deptList.map((dept) => {
            const count = rawUsers.filter((u: any) => u.departmentName === dept.name).length;
            return (
              <div 
                key={dept._id} 
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="pr-12 flex-1">
                    ...
                    {dept.managerName ? (
                      ...
                      <p className="text-xs font-medium text-gray-500 mt-1">Manager: <span className="text-gray-700">{dept.managerId}</span></p>
                    ) : null}
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{deptUsers.length} Employee{deptUsers.length !== 1 && 's'}</p>
                </div>
              </div>
              <button onClick={() => setSelectedDept(null)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-2 max-h-[60vh] overflow-y-auto">
              {deptUsers.length === 0 ? (
                ...
                <div className="space-y-1">
                  {deptUsers.map((u: any) => (
                    <div key={u._id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                      <div className="flex items-center gap-3">
        ...
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="text-base font-semibold text-gray-900">{isEditing ? "Edit Department" : "New Department"}</h2>
            ...
            <form onSubmit={(e) => { 
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wide">Parent Department</label>
              ...
              <div className="flex gap-3 pt-3">
                ...
                <button type="submit" disabled={createDept.isPending || updateDept.isPending} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-50">
                  {createDept.isPending || updateDept.isPending ? "Saving..." : (isEditing ? "Save Changes" : "Create Department")}
                </button>
              </div>
            </form>
          </div>
