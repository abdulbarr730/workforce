...
export default function DevicesPage() {
  ...
  const unassignMut = useMutation({
    mutationFn: (deviceId: string) => api.patch(`/api/devices/${deviceId}/unassign`),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider">Online (5m)</p>
            <p className="text-2xl font-bold">{online}</p>
          </div>
        </div>
        <div className="card p-5 flex items-center gap-4">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white" style={{ background: "linear-gradient(135deg,#f59e0b,#b45309)" }}>
            <UserPlus className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-wider">Assigned</p>
            <p className="text-2xl font-bold">{assigned}<span className="text-base text-gray-400 font-medium">/{total}</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center text-sm text-gray-400">Loading devices…</div>
                ...
                return (
                  <tr key={d._id}>
                    <td>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-gray-900 truncate">{d.hostname || "Unknown"}</p>
                          <p className="text-xs text-gray-500 font-mono truncate">{d.deviceId}</p>
                        </div>
                      </div>
                    </td>
                    <td>
                      <span className="text-sm">{d.os || "—"}</span>
                      {d.agentVersion && <span className="block text-[10px] text-gray-400">v{d.agentVersion}</span>}
                    </td>
                    <td>
                      {d.employee ? (
                        <div>
                          <p className="font-medium text-gray-900">{d.employee.name}</p>
                          <p className="text-xs text-gray-500">{d.employee.employeeId} · {d.employee.departmentName || "—"}</p>
                        </div>
                      ) : (
                    ...
                    <td>
                      ...
                      {d.lastEventType && <span className="block text-[10px] text-gray-400 uppercase tracking-wider">{d.lastEventType}</span>}
                    </td>
                    <td>
                      {online ? (
                        <span className="chip chip-green"><Wifi className="w-3 h-3" /> Online</span>
                      ) : (
                        <span className="chip chip-gray"><WifiOff className="w-3 h-3" /> Offline</span>
                      )}
                    </td>
                    <td className="text-right pr-6">
                      <div className="inline-flex gap-2">
                        ...
                        {d.employeeId ? (
                          ...
                          ><UserMinus className="w-3.5 h-3.5 inline mr-1" />Unassign</button>
                        ) : (
                          <button
                            onClick={() => { setAssignFor(d); setSelectedEmp(""); }}
      ...
      {/* Assign modal */}
      {assignFor && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setAssignFor(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-bold text-gray-900">Assign device</h3>
                <p className="text-xs text-gray-500 font-mono mt-1">{assignFor.hostname} · {assignFor.deviceId}</p>
              </div>
              <button onClick={() => setAssignFor(null)} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Employee</label>
            <select
            ...
            {assignMut.isError && <p className="text-xs text-red-600 mb-3">Failed to assign. Try again.</p>}
            <div className="flex gap-2 justify-end">
              <button onClick={() => setAssignFor(null)} className="btn-ghost">Cancel</button>
              <button
                onClick={() => selectedEmp && assignMut.mutate({ deviceId: assignFor.deviceId, employeeId: selectedEmp })}
      ...
      {/* View modal */}
      {viewDevice && (
        <DeviceDetailModal device={viewDevice} onClose={() => setViewDevice(null)} />
      )}
    </div>
  );
}
...
function DeviceDetailModal({ device, onClose }: { device: Device; onClose: () => void }) {
  ...
  const { data: analytics } = useQuery({
    ...
    queryFn: () =>
      api.get(`/api/analytics/employee-daily?employeeId=${device.employeeId}&date=${today}`)
        .then((r) => r.data.data)
        .catch(() => null),
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                {online ? <span className="chip chip-green"><Wifi className="w-3 h-3" /> Online</span>
                  : <span className="chip chip-gray"><WifiOff className="w-3 h-3" /> Offline</span>}
                <span className="chip chip-indigo"><Cpu className="w-3 h-3" /> {device.os || "Unknown OS"}</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-6 space-y-6">
          {/* Device metadata */}
          <section>
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Device</h4>
          ...
          {/* Employee */}
          <section>
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Assigned employee</h4>
            {device.employee ? (
              <div className="card p-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">{device.employee.email} · {device.employee.employeeId}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{device.employee.departmentName || "No department"} · {device.employee.role}</p>
                </div>
              </div>
            ) : (
              <div className="text-sm text-gray-500 italic">No employee assigned.</div>
            )}
          </section>
          ...
          {device.shiftPolicy && (
            <section>
              ...
              <div className="card p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <Detail label="Shift window" value={device.shiftPolicy.shiftStart && device.shiftPolicy.shiftEnd
                    ? `${device.shiftPolicy.shiftStart} – ${device.shiftPolicy.shiftEnd}` : "—"}
                    icon={<Clock className="w-3 h-3" />} />
                  <Detail label="Working days" value={(device.shiftPolicy.workingDays ?? []).join(", ") || "—"}
          ...
          {device.employeeId && (
            <section>
              <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3">Today&apos;s activity</h4>
              {analytics ? (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <MiniStat label="Productive" value={analytics.productiveMinutes ?? analytics.productive ?? "—"} suffix="min" tone="green" />
                  <MiniStat label="Unproductive" value={analytics.unproductiveMinutes ?? analytics.unproductive ?? "—"} suffix="min" tone="rose" />
                  <MiniStat label="Neutral" value={analytics.neutralMinutes ?? analytics.neutral ?? "—"} suffix="min" tone="gray" />
                  <MiniStat label="Score" value={analytics.productivityScore ?? "—"} suffix="" tone="indigo" />
                </div>
              ) : (
                <div className="text-sm text-gray-500 italic">No activity recorded today yet.</div>
              )}
            </section>
          )}
        </div>
...
function Detail({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return (
    <div>
      <p className="text-[10px] text-gray-500 uppercase tracking-wider flex items-center gap-1">{icon}{label}</p>
      <p className="text-sm text-gray-900 mt-0.5 font-medium">{value}</p>
    </div>
  );
}

function MiniStat({ label, value, suffix, tone }: { label: string; value: string | number; suffix: string; tone: "green" | "rose" | "gray" | "indigo" }) {
  ...
  return (
    <div className={`rounded-xl p-3 ${tones[tone]}`}>
      ...
      <p className="text-lg font-bold mt-1">{value}<span className="text-xs font-medium ml-1 opacity-75">{suffix}</span></p>
    </div>
  );
}
