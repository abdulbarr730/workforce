...
export default function ProductivityRulesPage() {
  ...
  const createRule = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["productivity-rules"] }); closeForm(); },
  });

  const updateRule = useMutation({
    mutationFn: (payload: any) => api.put(`/api/productivity-rules/${editingId}`, payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["productivity-rules"] }); closeForm(); },
  });
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...form, scopeId: form.scopeType === "GLOBAL" ? null : form.scopeId || null };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Classify apps and patterns with specific scopes and scores.</p>
        </div>
                ...
                {ruleList.map((rule) => (
                  <tr key={rule._id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="px-4 py-3">
                      ...
                      {rule.titlePattern && <p className="text-xs text-gray-400 font-mono mt-0.5">{rule.titlePattern}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-xs font-medium text-gray-700">{rule.scopeType}</p>
                      {rule.scopeType !== "GLOBAL" && <p className="text-xs text-gray-400">{rule.scopeId}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CATEGORY_COLORS[rule.productivityCategory] ?? "bg-gray-100 text-gray-600"}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => confirm("Delete this rule?") && deleteRule.mutate(rule._id)} className="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 my-8">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold text-gray-900">{editingId ? "Edit Rule" : "Add Rule"}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.scopeType} onChange={(e) => setForm({ ...form, scopeType: e.target.value as any, scopeId: "" })}
              ...
              <div>
                ...
                <input type="text" required value={form.appName} onChange={(e) => setForm({ ...form, appName: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" placeholder="e.g. Chrome, VS Code" />
              </div>
              
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Title / URL Pattern (Regex/Substring)</label>
                <input type="text" value={form.titlePattern || ""} onChange={(e) => setForm({ ...form, titlePattern: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" placeholder="e.g. youtube\.com, Google Sheets" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Category</label>
                  <select value={form.productivityCategory} onChange={(e) => setForm({ ...form, productivityCategory: e.target.value as any })}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
                    {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Productivity Score (0 to 1)</label>
                  <input type="number" step="0.1" min="0" max="1" required value={form.productivityScore} onChange={(e) => setForm({ ...form, productivityScore: parseFloat(e.target.value) })}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={createRule.isPending || updateRule.isPending} className="flex-1 py-2 bg-gray-900 text-white font-medium rounded-lg text-sm hover:bg-gray-800 disabled:opacity-50">
