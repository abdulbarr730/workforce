...
import { 
  ...
  MonitorOff, LineChart, Link as LinkIcon, UserX, UserCheck, FileText, ClipboardList
} from "lucide-react";
        ...
        <div className="flex items-center gap-3 relative">
          ...
          <button 
            onClick={async () => {
              ...
              if (btn) btn.innerHTML = '<span class="animate-pulse">Syncing... (may take 20s)</span>';
              try {
                ...
                setTimeout(() => { if (btn) btn.innerHTML = 'Force Sync Data' }, 3000);
              }
            }} 
            id="sync-btn"
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="p-2 flex-1 max-h-[300px] overflow-y-auto">
                  {intelligence?.compliance?.missedOneDay?.length === 0 ? (
                    <p className="text-sm text-gray-500 text-center py-6 font-medium">All clear! Everyone submitted EODs.</p>
                  ) : (
                    <ul className="space-y-1">
                      {intelligence?.compliance?.missedOneDay?.map((emp: any) => (
                        <li key={emp.employeeId} className="flex justify-between items-center p-3 rounded-xl hover:bg-gray-50 transition-colors">
                          <span className="text-sm font-bold text-gray-900">{emp.name}</span>
                          <span className="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs font-bold">1 Day Missed</span>
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="p-2 flex-1 max-h-[300px] overflow-y-auto">
                  {intelligence?.compliance?.missedMultipleDays?.length === 0 ? (
                    <p className="text-sm text-gray-500 text-center py-6 font-medium">All clear! No repeat offenders.</p>
                  ) : (
                    <ul className="space-y-1">
                      {intelligence?.compliance?.missedMultipleDays?.map((emp: any) => (
                        <li key={emp.employeeId} className="flex justify-between items-center p-3 rounded-xl hover:bg-gray-50 transition-colors">
                          <span className="text-sm font-bold text-gray-900">{emp.name}</span>
                          <span className="px-2.5 py-1 bg-red-100 text-red-700 rounded-lg text-xs font-black">{emp.eodsMissed} Days Missed</span>
                        </li>
                      ))}
                    </ul>
                  )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center justify-center">
                ...
                <p className="text-xs font-bold text-gray-500">Total EODs Submitted</p>
                <p className="text-2xl font-black text-gray-900 mt-1">{stats.totalEods || 0}</p>
              </div>
              <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col items-center justify-center">
                <CheckCircle2 className="w-6 h-6 text-emerald-400 mb-2" />
                <p className="text-xs font-bold text-gray-500">Total Todos Created</p>
                <p className="text-2xl font-black text-gray-900 mt-1">{stats.totalTodos || 0}</p>
              </div>
            </div>
          </div>
        </>
