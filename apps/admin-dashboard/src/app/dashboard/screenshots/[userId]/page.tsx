  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Employee Screenshots</h1>
      ...
      {isLoading ? (
        ...
        <div className="p-6 bg-red-50 text-red-700 rounded-xl border border-red-100">
          Failed to load screenshots. Make sure you have the correct permissions.
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {screenshots.map((s: any) => {
            const time = new Date(s.capturedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
            return (
              <div key={s._id} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <a href={s.imageUrl} target="_blank" rel="noreferrer" className="block relative group aspect-video bg-gray-100">
                  <img 
                    src={s.imageUrl} 
                    alt="Screenshot" 
