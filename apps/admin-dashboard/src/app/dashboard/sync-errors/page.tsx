...
export default function SyncErrorsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Mark as read
        await axios.put(`${API_URL}/devices/errors/mark-read`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mt-12 mb-8 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Device Crash & Network Logs</h2>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            A log of errors reported by Desktop Agents, such as database corruption, invalid tokens, or uncaught exceptions.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-12 text-center mb-12">
          <h3 className="text-lg font-medium text-purple-900">No Device Errors</h3>
          <p className="mt-1 text-sm text-purple-600">All Desktop Agents are running smoothly with no crashes reported.</p>
        </div>
      ) : (
        <div className="bg-white dark:bg-gray-800 shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl overflow-hidden mb-12">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-900/50">
              <tr>
                <th className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-200 sm:pl-6">Received At</th>
                <th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-200">Device</th>
                <th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-200">Type</th>
                <th className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-200">Message & Stack</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {deviceErrors.map((error) => (
                <tr key={error._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm align-top">
                    <div className="font-medium text-gray-900 dark:text-white">Emp: {error.employeeId || 'Unknown'}</div>
                    <div className="text-gray-500 dark:text-gray-400 mt-1 text-xs font-mono bg-gray-100 dark:bg-gray-900 px-2 py-1 rounded inline-block">
                      {error.deviceId}
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-4 text-sm text-gray-500 font-mono align-top">
                    <p className="text-sm font-medium text-gray-900 mb-2 whitespace-pre-wrap">{error.errorMessage}</p>
                    {error.stackTrace && (
                      <details className="max-w-xl max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-2 rounded border border-gray-200 dark:border-gray-700 text-xs">
                        <summary className="cursor-pointer text-blue-600 hover:text-blue-800">View Stack Trace</summary>
                        <pre className="mt-2 text-[10px] whitespace-pre-wrap">{error.stackTrace}</pre>
                      </details>
                    )}
                  </td>
                </tr>
