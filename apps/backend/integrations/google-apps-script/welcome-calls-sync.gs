...
function syncStatusToWorkforce(e) {
  ...
  var properties = PropertiesService.getScriptProperties();
  var currentWeeklySheet =
    properties.getProperty("WELCOME_CALL_CURRENT_WEEKLY_SHEET") ||
    "Weekly Masterclass";
  if (sheet.getName() !== currentWeeklySheet) return;
  ...
  if (response.getResponseCode() < 200 || response.getResponseCode() >= 300) {
    throw new Error(
      "Workforce status sync failed: " + response.getContentText(),
    );
  }
}
