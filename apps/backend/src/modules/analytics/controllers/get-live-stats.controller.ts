...
// Every desktop agent polls live stats (the shift watcher every few seconds)
// and each call replays the whole day of telemetry. Identical requests within
// the TTL share one computation.
const LIVE_STATS_TTL_MS = 15_000;
const liveStatsCache = new MicroCache<Record<string, unknown>>(
  LIVE_STATS_TTL_MS,
);

export const invalidateLiveStatsCache = (employeeId?: string) =>
  liveStatsCache.invalidate(
    employeeId ? (key) => key.startsWith(`${employeeId}|`) : undefined,
  );
...
export const getLiveStatsController = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    const data = await liveStatsCache.getOrCompute(
      `${employeeId}|${date}`,
      () => buildLiveStats(employeeId, date),
    );

    return res.json(successResponse(data, "Live stats fetched"));
  },
);

const buildLiveStats = async (employeeId: string, date: string) => {
  ...
  const rawEvents = await ActivityEvent.find({
    ...
    .select("type timestamp productivityCategory metadata")
    .sort({ timestamp: 1 })
    .lean();
  const now = new Date();
  ...
  const staleOvernightLogin =
    ...
    firstRealActivityDate.getTime() - storedLoginDate.getTime() > 120 * 60_000;
  ...
  for (const ev of events) {
    ...
    if (ev.type === "ACTIVE_WINDOW") {
      ...
      if (tsStart.getTime() < activeCoveredUntil) {
        ...
        actualDur = Math.max(0, (tsEnd.getTime() - activeCoveredUntil) / 1000);
      }
  ...
  const totalDeduction =
    idleSeconds + breakOvertimeSeconds + offlineWorkSeconds;
