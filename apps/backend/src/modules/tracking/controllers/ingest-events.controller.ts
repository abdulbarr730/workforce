...
import { ingestEventsSchema, singleEventSchema } from "../validators/ingest-events.validator";
...
export const ingestEventsController = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Received batch of ${validatedData.body.events.length} tracking events`);
    ...
    for (const rawEvent of validatedData.body.events) {
      ...
      if (parseResult.success) {
        ...
        console.error("Event failed strict validation, pushing to Sync Errors log");
    ...
    if (validEvents.length > 0) {
      ...
      console.log(`Successfully ingested ${validEvents.length} valid events.`);
    }

    // 5. ALWAYS return 201 so the desktop agent clears its queue!
    return res.status(201).json(
      successResponse(result, "Batch processed successfully")
    );
    
  } catch (error) {
    if (error instanceof ZodError) {
      // This only triggers if the base array structure is invalid
      return res.status(400).json({ success: false, message: "Invalid payload structure", errors: error.issues });
    }
