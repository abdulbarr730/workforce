...
import { startScreenshotTracker, stopScreenshotTracker } from "./screenshot.tracker";
...
function isWithinSchedule(): boolean {
  ...
  const [startH, startM] = trackingState.trackingStartTime.split(":").map(Number);
...
export function checkSchedule() {
  ...
  if (!allowedNow && isCurrentlyAllowed) {
    ...
    import("electron").then(({ BrowserWindow }) => {
      BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("tracking:schedule-paused"));
    });
    ...
    // Notify UI
    import("electron").then(({ BrowserWindow }) => {
      BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("tracking:schedule-resumed"));
    });
  }
}
