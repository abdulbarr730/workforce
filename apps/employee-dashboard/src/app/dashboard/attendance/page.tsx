...
export default function MyAttendancePage() {
  ...
  const { data: records, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/api/attendance/records?employeeId=${user?.employeeId}&month=${month}`).then((r) => r.data.data),
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-sm text-gray-400">No records for this month</div>
        ) : (
              ...
              <tbody>
                {list.map((record) => (
                  <tr key={record._id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{formatDate(record.date)}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getStatusColor(record.attendanceStatus)}`}>
                        {record.attendanceStatus}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">{record.loginTime ? new Date(record.loginTime).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) : "—"}</td>
