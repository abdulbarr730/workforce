...
export default function MyLeavesPage() {
  ...
  const requestLeave = useMutation({
    mutationFn: (payload: typeof form) => api.post("/api/attendance/time-off/leaves/request", payload),
    onSuccess: () => {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{leaveList.filter((l) => l.status === "PENDING").length} pending</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); requestLeave.mutate(form); }} className="space-y-4">
              <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={requestLeave.isPending} className="flex-1 py-2 bg-gray-900 text-white rounded-lg text-sm hover:bg-gray-800 disabled:opacity-50">
