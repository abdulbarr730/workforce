...
export default function EmployeeDashboardPage() {
  ...
  const { data: dailyAnalytics } = useQuery({
    ...
    queryFn: () => api.get(`/api/me/analytics?date=${today}`).then((r) => r.data.data),
    enabled: !!user,
  });

  const { data: activeSession } = useQuery({
    queryKey: ["my-session"],
    queryFn: () => api.get("/api/work-sessions/active").then((r) => r.data.data),
    enabled: !!user,
  });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {dailyAnalytics?.attendanceStatus ? (
            <span className={`text-sm px-2 py-0.5 rounded-full font-medium ${getStatusColor(dailyAnalytics.attendanceStatus)}`}>
              {dailyAnalytics.attendanceStatus}
            </span>
          ) : (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p className="text-2xl font-semibold text-gray-900">
            {dailyAnalytics ? formatMinutes(dailyAnalytics.productiveMinutes ?? 0) : "—"}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p className="text-2xl font-semibold text-gray-900">
            {dailyAnalytics ? formatMinutes(dailyAnalytics.breakMinutes ?? 0) : "—"}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-gray-900">{formatDate(activeSession.loginTime)} {new Date(activeSession.loginTime).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Session ID</span>
              <span className="text-gray-600 font-mono text-xs">{activeSession.sessionId?.slice(0, 16)}...</span>
            </div>
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {activeSession.todos.map((t: string, i: number) => (
                    <li key={i} className="text-gray-700 text-xs">• {t}</li>
                  ))}
                </ul>
              </div>
            )}
